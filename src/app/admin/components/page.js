...
export default function AdminComponentsPage() {
    ...
    if (loading) {
        return <div className="flex justify-center p-12"><Loader2 className="animate-spin text-neon-green" /></div>;
    }
                    ...
                    <tbody className="divide-y divide-white/5">
                        {components.map((comp) => (
                            <tr key={comp._id} className="hover:bg-white/5 transition-colors">
                                ...
                                <td className="p-4 text-gray-400 max-w-md truncate">{comp.description}</td>
