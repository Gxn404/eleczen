      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 tracking-tight">
            Community <span className="text-neon-pink">Projects</span>
          </h1>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filter === cat
                    ? "bg-neon-pink text-black shadow-[0_0_15px_rgba(255,0,128,0.4)]"
          ...
          <button className="px-8 py-3 rounded-xl bg-white/10 text-white font-bold hover:bg-white/20 transition-all border border-white/10 hover:border-neon-pink/50">
            Submit Your Project
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            <div
              key={project.id}
              className="glass-panel rounded-2xl overflow-hidden border border-white/10 hover:border-neon-pink/30 transition-all group"
            >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                  <div className="flex gap-2">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-1 rounded-md bg-neon-pink/20 text-neon-pink text-xs font-bold backdrop-blur-sm border border-neon-pink/20"
                      >
                        {tag}
                      </span>
                    ))}
