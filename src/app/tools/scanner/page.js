...
export default function ComponentScanner() {
    ...
    const handleScan = async (imageSrc) => {
        ...
        try {
            ...
            if (cleanText.includes("RES") || cleanText.match(/\d+K/) || cleanText.match(/\d+R/)) {
                identification = "Resistor Detected";
                details = "Found patterns matching resistor markings.";
            } else if (cleanText.includes("CAP") || cleanText.includes("µF") || cleanText.match(/\d+V/)) {
                identification = "Capacitor Detected";
                details = "Found voltage or capacitance ratings.";
            } else if (cleanText.includes("IC") || cleanText.match(/LM\d+/) || cleanText.match(/NE\d+/)) {
        ...
        <div className="min-h-screen py-12 px-4">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-400">Point your camera at a component to identify it using AI.</p>
                </div>
                    ...
                    <div className="space-y-6">
                        ...
                        {result && !scanning && (
                            <div className="glass-panel rounded-2xl p-6 border border-neon-pink/30 relative overflow-hidden">
                                ...
                                <h3 className="text-xl font-bold text-white mb-1">{result.identification}</h3>
                                <p className="text-neon-pink text-sm mb-4 font-mono">{result.details}</p>

                                <div className="bg-black/40 rounded-lg p-4 mb-4 border border-white/5">
                                    <p className="text-xs text-gray-500 uppercase mb-1">Raw OCR Text</p>
